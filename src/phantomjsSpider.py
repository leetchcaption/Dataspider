...
'''
reference from http://www.jianshu.com/p/3d84afc43d42
'''
...
class Spider:
    ...
    def loadPageContent(self,page):
        ...
        pattern = re.compile(r'<div.*?class="pic-word">.*?<img src="(.*?)".*?'
        ...
        for item in items:
            # 头像，个人详情，名字，年龄，地区
            print(u'发现一位MM 名字叫%s 年龄%s 坐标%s'
            ...
            detailPage = item[1]
            name = item[2]
            self.getDetailPage(detailPage, name, beginTime)

    def getDetailPage(self,url, name, beginTime):
        url = 'http:' + str(url)
        self.driver.get(url)
        ...
        try:
            ...
            print(u'获取相册信息异常... %s' % e)
        endTime = datetime.datetime.now()
        # 保存个人信息以及耗时
        try:
            self.saveBrief(brief, dir, name, endTime - beginTime)
        except Exception as e:
